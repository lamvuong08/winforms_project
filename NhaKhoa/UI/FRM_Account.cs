    ...
    public partial class FRM_Account : Form
    {
        ...
        private void LoadRoles()
        {
            try
            {
                ...
                clb_role.Items.Clear();
                foreach (var role in list.OrderBy(x => x.Name))
                {
                    clb_role.Items.Add(new RoleItem { Id = role.Id, Name = role.Name });
                }
            }
            catch (Exception ex)
            {
        ...
        private void ClearForm()
        {
            ...
            for (int i = 0; i < clb_role.Items.Count; i++)
                clb_role.SetItemChecked(i, false);
        }
        ...
        private void LoadUserRoles(int userId)
        {
            try
            {
                ...
                foreach (var ur in userRoles)
                {
                    for (int i = 0; i < clb_role.Items.Count; i++)
                    {
                        var item = clb_role.Items[i] as RoleItem;
                        if (item != null && item.Id == ur.RoleId)
                            clb_role.SetItemChecked(i, true);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void btn_addUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    // Gán roles cho user
                    foreach (RoleItem item in clb_role.CheckedItems)
                    {
                        _userRoleBus.GanRoleChoUser(newUser.Id, item.Id);
                    }
                }
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (RoleItem item in clb_role.CheckedItems)
                {
                    _userRoleBus.GanRoleChoUser(currentUserId, item.Id);
                }
